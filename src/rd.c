...
bool swapchain_init(Arena *arena, Application *app) {
  ...
  app->swapchain_format = available_formats[format_index];
...
static void rd_cleanup(Application *app) {
  ...
  if (app->descriptor_sets != NULL)
    vkFreeDescriptorSets(app->device, app->descriptor_pool, app->inflight_count,
                         app->descriptor_sets);
...
bool rd_create_pipeline(Application *app) {
  ...
  VkViewport viewport = {.x = 0.0f,
                         ...
                         .minDepth = 0.0f,
                         .maxDepth = 1.0f};
  VkRect2D scissor = {.offset = (VkOffset2D){0, 0}, .extent = app->swap_extent};
  ...
  VkPipelineRenderingCreateInfo pipeline_rendering_info = {
      ...
      .pColorAttachmentFormats = &app->swapchain_format.format,
...
bool rd_create_descriptor_set(Application *app) {
  ...
  app->descriptor_sets =
      ARENA_PUSH_ARRAY(app->vulkan_arena, app->inflight_count, VkDescriptorSet);
...
bool rd_upload_bitmap(Application *app, u8 *bitmap, u32 width, u32 height) {
  ...
  transition_image_layout(
      ...
      VK_QUEUE_FAMILY_IGNORED, VK_IMAGE_ASPECT_COLOR_BIT);

  copy_buffer_to_image(app->transfer_command_buffer, width, height,
  ...
  transition_image_layout(app->transfer_command_buffer, app->texture_image,
                          ...
                          app->graphic_queue_index, VK_IMAGE_ASPECT_COLOR_BIT);
...
// @format
bool find_supported_format(Application *app, VkFormat *candidates,
                           u32 candidate_count, VkImageTiling tiling,
                           VkFormatFeatureFlags features, VkFormat *out) {
  for (u32 i = 0; i < candidate_count; i++) {
    VkFormatProperties properties;
    vkGetPhysicalDeviceFormatProperties(app->physical_device, candidates[i],
                                        &properties);
    if ((tiling == VK_IMAGE_TILING_LINEAR &&
         (features & properties.linearTilingFeatures) == features) ||
        (tiling == VK_IMAGE_TILING_OPTIMAL &&
         (features & properties.optimalTilingFeatures) == features)) {
      *out = candidates[i];
      return true;
    }
  }
...
i32 rd_begin_rendering(Application *app,
  ...
  *current_command_buffer = app->graphic_command_buffers[app->frame_index];
  ...
  transition_image_layout(
      *current_command_buffer, (app->swapchain_images)[image_index],
      ...
      VK_QUEUE_FAMILY_IGNORED, VK_IMAGE_ASPECT_COLOR_BIT);
  transition_image_layout(*current_command_buffer, app->depth_image,
